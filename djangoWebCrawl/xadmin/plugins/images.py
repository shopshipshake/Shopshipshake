...
class AdminImageWidget(forms.FileInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        if value and hasattr(value, "url"):
            ...
            output.append('<a href="%s" target="_blank" title="%s" data-gallery="gallery"><fix_img src="%s" class="field_img"/></a><br/>%s ' %
    ...
    def get_field_result(self, result, field_name):
        if isinstance(result.field, models.ImageField):
            if result.value:
                ...
                result.text = mark_safe('<a href="%s" target="_blank" title="%s" data-gallery="gallery"><fix_img src="%s" class="field_img"/></a>' % (img.url, result.label, img.url))
                self.include_image = True
        return result
